...
/**
 * Class Phone
 * @package xomorg\helpers
 */
class Phone
{

    /**
     * @var array Phone codes
     */
    ...
    /**
     * @param string $phone
     * @param array $params params: '{country} {phone}'
     *
     * @return string
     */
    public static function format($phone = '', $params = array())
    {
        if (empty($phone)) {
            return '';
        }

        if (! isset($params['template'])) $params['template'] = '{phone}';
        if (! isset($params['convert'])) $params['convert'] = true;
        if (! isset($params['removeBrackets'])) $params['removeBrackets'] = false;
        if (! isset($params['removePlus'])) $params['removePlus'] = false;
        if (! isset($params['replace8-7'])) $params['replace8-7'] = false;

        // очистка от лишнего мусора с сохранением информации о "плюсе" в начале номера
        $phone=trim($phone);
        $plus = (($phone[0] == '+') xor $params['removePlus']);

        $phone = preg_replace("/[^0-9A-Za-z]/", "", $phone);
        $OriginalPhone = $phone;

        // конвертируем буквенный номер в цифровой
        if ($params['convert'] == true && !is_numeric($phone)) {
        ...
        if (strlen($phone)>7) {
            foreach (self::$phoneCodes as $countryCode => $data) {
                ...
                if (substr($phone, 0, $codeLen) == $countryCode) {
                    ...
                    if ($params['replace8-7'] and $countryCode=='8') {
                        $countryCode = '7';
                        $plus = (true xor $params['removePlus']);
                    }

                    // возвращаем результат
                    $phone = ($plus ? "+" : "") . $countryCode . ' ' . (! $params['removeBrackets'] ? '(' : '') . $cityCode . (! $params['removeBrackets'] ? ')' : '') . ' ' . self::toBlocks($phone);
                    if ($country)
                    {
                        $params['template'] = str_replace('{country}', $country['name'], $params['template']);
                    }

                    return str_replace('{phone}', $phone, $params['template']);
                }
            }
        }

        $phone = ($plus ? "+" : "").self::toBlocks($phone);
        if ($country)
        {
            $params['template'] = str_replace('{country}', $country['name'], $params['template']);
        }
     ...
     * @return string
     */
    public static function toBlocks($number)
    {
        $add='';
        if (strlen($number)%2)
        {
